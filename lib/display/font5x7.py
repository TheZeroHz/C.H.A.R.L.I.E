# lib/display/font5x7.py
# Compact 5x7 ASCII font (cols packed in 5 bytes per glyph, bit0 = top).

# fmt: glyphs for ' ' (32) .. '~' (126)
# Each glyph: 5 bytes = 5 columns, LSB = top pixel.
...
def draw_text(fb, s, x, y, color=1):
    """Draw string with 5x7 font onto a FrameBuffer-like object (pixel method)."""
    if s is None:
        return
    for ch in str(s):
        o = ord(ch)
